...
namespace ASTMViewer
{
    public partial class ASTMParserGUI : Form
    {
        ...
        void populateData()
        {
            ...
            // Call the method to print the tree nodes
            PrintTreeNodes(messagesData.Nodes);
        }

        private void PrintTreeNodes(TreeNodeCollection nodes, string indent = "")
        {
            foreach (TreeNode node in nodes)
            {
                Debug.WriteLine(indent + node.Text);
                if (node.Nodes.Count > 0)
                {
                    PrintTreeNodes(node.Nodes, indent + "  ");
                }
            }
        }
